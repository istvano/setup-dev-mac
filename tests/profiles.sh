...
#
# No `uname` stub any more. This used to need one, because `all` subtracted the
# profiles script/platform-gaps reported as unavailable and the answer therefore
# depended on the host architecture. Intel support is gone (ADR-036), so `all`
# means all everywhere and the test says so without faking the machine.
all_profiles="$("$ROOT/bootstrap" plan --profiles all 2>/dev/null |
  sed -n 's/^  profiles: *//p')"
[[ -n "$all_profiles" ]] ||
  fail "'./bootstrap plan --profiles all' printed no profile line."
for profile in "${VALID_PROFILES[@]}"; do
  case ",$all_profiles," in
    *",$profile,"*) ;;
    *) fail "--profiles all omitted '$profile'. It expands from VALID_PROFILES,
so every profile must appear." ;;
  esac
done
